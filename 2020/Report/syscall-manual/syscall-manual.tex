...
\section{システムコール追加の手順}\label{sec:tejun}
 \subsection{概要}
以下でシステムコール追加の手順を示す．まず，Linux カーネルを取得する．次にソースコードの編集を行う．最後にカーネルの再構築を行う．以降では上記の手順について述べる．
...
\end{verbatim}
  ...
  以下の手順にしたがってカーネルの再構築を行う．コマンドは/home/nakagawa/git/linux-stable 以下で実行する．
  \begin{enumerate}
  \item .config ファイルの生成\\
